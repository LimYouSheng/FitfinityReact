...
import { EXERCISE_MEDIA_TYPES, exerciseCategories, exerciseDraftErrors, exerciseMediaError, filterExerciseCatalog } from '../../app/exerciseCatalog.js'
import ExerciseLibraryMedia from './ExerciseLibraryMedia.jsx'

const EXERCISE_EDIT_LABEL = 'Exercise library entry'
const draftFor = exercise => ({ name: exercise?.name ?? '', category: exercise?.category ?? exerciseCategories[0], description: exercise?.description ?? '', status: exercise?.status ?? 'active' })
const mediaLabel = media => media ? media.type.startsWith('image/') ? 'Photo' : 'Video' : '—'

function ExerciseDetail({ exercise, exercises, creating, onSave, onNavigate }) {
  const confirm = useActionConfirmation()
  const { activeEdit, setActiveEdit } = useEditGuard()
  const [editing, setEditing] = useState(creating)
  const [draft, setDraft] = useState(() => draftFor(exercise))
  ...
  const dirty = JSON.stringify(draft) !== JSON.stringify(draftFor(exercise)) || Boolean(file) || removeMedia
  useEffect(() => {
    setActiveEdit(editing && dirty ? EXERCISE_EDIT_LABEL : null)
    return () => setActiveEdit(null)
  }, [dirty, editing, setActiveEdit])
  useEffect(() => {
    if (!editing) setDraft(draftFor(exercise))
  }, [editing, exercise])
  useEffect(() => {
  ...
  const persist = async (nextDraft, statusOnly = false) => {
    ...
    const validation = exerciseDraftErrors(nextDraft, exercises, exercise?.id)
  ...
  if (!editing) return <Panel>
    <div className="section-head"><div><h2>{exercise?.name ?? created?.name}</h2><StatusBadge tone={exercise?.status === 'inactive' ? 'neutral' : 'green'}>{exercise?.status === 'inactive' ? 'Inactive' : 'Active'}</StatusBadge></div>
      <button type="button" className="text-action" disabled={busy} onClick={() => { setEditing(true); setDraft(draftFor(exercise)) }}>Edit Exercise</button>
    </div>
    ...
    <ExerciseLibraryMedia media={exercise?.media} />
    {error && <p className="validation-copy" role="alert">{error}</p>}
    {exercise && <div className="library-lifecycle"><button className="secondary-button" type="button" disabled={busy} onClick={() => persist({ ...draftFor(exercise), status: exercise.status === 'active' ? 'inactive' : 'active' }, true)}>{exercise.status === 'active' ? 'Deactivate Exercise' : 'Reactivate Exercise'}</button></div>}
  </Panel>
  ...
  return <Panel>
    <form className="library-form" noValidate onSubmit={event => { event.preventDefault(); persist(draft) }}>
      <fieldset disabled={busy}>
        ...
        <div className={`library-field is-required ${errors.category ? 'is-invalid' : ''}`}><label htmlFor="library-category">Category <span aria-hidden="true">*</span></label><select id="library-category" required value={draft.category} aria-invalid={Boolean(errors.category)} onChange={event => patch({ category: event.target.value })}>{exerciseCategories.map(category => <option key={category}>{category}</option>)}</select>{errors.category && <small>{errors.category}</small>}</div>
        ...
        <ExerciseLibraryMedia file={file} media={removeMedia ? null : exercise?.media} />
...
function OwnerExerciseLibrary({ exercises, detailId, onNavigate, onBack, onSave }) {
  ...
  return <div className={`exercise-library-page${activeEdit === EXERCISE_EDIT_LABEL ? ' editing-section' : ''}`}>
    ...
    {detailId ? creating || selected ? <ExerciseDetail key={detailId} exercise={selected} exercises={exercises} creating={creating} onSave={onSave} onNavigate={onNavigate} /> : <Panel><p>Exercise not found.</p></Panel>
      : <Panel>
        <div className="library-filters"><input type="search" aria-label="Search exercises" placeholder="Search exercises" value={query} onChange={event => setQuery(event.target.value)} /><select aria-label="Exercise category" value={category} onChange={event => setCategory(event.target.value)}><option value="">All categories</option>{exerciseCategories.map(item => <option key={item}>{item}</option>)}</select><select aria-label="Exercise status" value={status} onChange={event => setStatus(event.target.value)}><option value="active">Active</option><option value="inactive">Inactive</option><option value="">All statuses</option></select></div>
