  ...
  return <main className="account-entry"><Panel>
    ...
    <form onSubmit={submit} className="account-form">
      <fieldset disabled={busy}>
        <label>Account{accounts.length ? <select aria-label="Account" required value={identifier} onChange={event => setIdentifier(event.target.value)}><option value="">Choose account</option>{accounts.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select> : <input aria-label="Account" autoComplete="username" required value={identifier} onChange={event => setIdentifier(event.target.value)} />}</label>
