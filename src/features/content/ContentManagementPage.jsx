...
function ContentEditor({ entry, onSave, onNavigate, onBack }) {
  ...
  if (saved) return <Panel><h2>{saved.title}</h2><p>{saved.key} · {statuses[saved.status]}</p><article className="content-preview"><p>{saved.body}</p></article><button type="button" className="secondary-button" onClick={() => { setSaved(null); setDraft(draftFor(saved)) }}>Edit Content</button><button type="button" className="secondary-button" onClick={onBack}>Back to Content</button></Panel>
...
export default function ContentManagementPage({ entries, detailId, onSave, onNavigate, onBack }) {
