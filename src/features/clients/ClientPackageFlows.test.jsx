...
it('edits client details with creation controls, validates before saving and retains a failed draft', async () => {
  ...
  expect(screen.getByLabelText('Client birthday')).toHaveAttribute('type', 'text')
...
it('uses creation controls and validation for trainer profile details and decimal rates', async () => {
  ...
  expect(screen.getByLabelText('Trainer type')).toHaveAttribute('aria-autocomplete', 'list')
  expect(screen.getByLabelText('Trainer birthday')).toHaveAttribute('type', 'text')
...
it('exports and shares only the selected package through the same PDF renderer while preserving compact charts', async () => {
  ...
  const title = document.querySelector('.package-progress-title')
  expect(title).toHaveClass('section-head')
  expect(title.children[0].tagName).toBe('H2')
  expect(title.children[1]).toHaveAttribute('aria-label', 'Package progress dates')
  expect(title.querySelector('time')).toHaveAttribute('datetime', source.package.startDate)
...
it('filters and paginates client history and upcoming sessions independently with inclusive dates', () => {
  ...
  expect(screen.getByLabelText('Session History from').closest('.date-filter-control')).not.toBeNull()
