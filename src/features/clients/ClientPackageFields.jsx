  ...
  return (
    <div className="onboarding-grid">
      <div className="onboarding-package-fields">
        <Field label="PT Package" required error={errors.packageId}><SelectField aria-label="PT Package" value={draft.packageId} onChange={event => {
          const item = packages.find(item => item.id === event.target.value)
          if (item) update({ packageId: item.id })
        }}><option value="" disabled>Choose package</option>{packages.map(item => <option value={item.id} key={item.id}>{item.name} · {item.validityDays} days</option>)}</SelectField></Field>
        <Field label="Start date" required error={errors.startDate}>
          <DateField aria-label="Start date"  min={draft.minimumStartDate} value={draft.startDate} onChange={event => update({ startDate: event.target.value })} />
        </Field>
        <Field label="Weekly frequency" required error={errors.sessionsPerWeek}>
          <SelectField aria-label="Weekly frequency" value={draft.sessionsPerWeek} onChange={event => {
      ...
      <Field label="Trainer preference">
        <SelectField aria-label="Trainer preference" value={draft.genderPreference} onChange={event => update({ genderPreference: event.target.value })}>
