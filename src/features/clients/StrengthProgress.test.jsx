...
async function prepareShare() {
  const button = screen.getByRole('button', { name: 'Share Progress Report PDF' })
...
it('retries failed export history without starting a second download', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Download Progress Report PDF' })).toBeDisabled()
...
it('downloads the prepared PDF for manual attachment when native file sharing is unavailable', async () => {
  ...
  const save = vi.fn().mockResolvedValue({})
  render(<StrengthProgress client={client} user={owner} onRecordAction={save} />)
  fireEvent.click(await prepareShare())
  const button = screen.getByRole('button', { name: 'Download Progress Report PDF' })
  await screen.findByText('PDF sharing is unavailable in this browser. Use Download PDF to share the file manually.')
...
it('rejects unsupported direct sharing without recording an event and keeps Download beside Share', async () => {
  const share = nativeShare(vi.fn(), vi.fn().mockReturnValue(false))
  const save = vi.fn()
  ...
  expect(button.previousElementSibling).toBe(screen.getByRole('button', { name: 'Download Progress Report PDF' }))
  expect(share).not.toHaveBeenCalled()
  expect(save).not.toHaveBeenCalled()
  expect(progressReportPdf).toHaveBeenCalledTimes(1)
  expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
...
it('retries PDF preparation and ignores late renders after changing packages or leaving Progress', async () => {
  const share = nativeShare(), save = vi.fn()
  let finish
  ...
  const view = render(<StrengthProgress client={client} user={owner} onRecordAction={save} />)
  ...
  view.rerender(<StrengthProgress client={{ ...client, reportPackageId: 'next' }} user={owner} onRecordAction={save} />)
  const button = await prepareShare()
  await act(async () => finish(new Blob(['old PDF'], { type: 'application/pdf' })))
  fireEvent.click(button)
  await waitFor(() => expect(save).toHaveBeenCalledTimes(1))
  expect(share.mock.calls[0][0].files[0].size).toBe(8)
  expect(save.mock.calls[0][0].packageId).toBe('next')
  let last
  vi.mocked(progressReportPdf).mockImplementationOnce(() => new Promise(resolve => { last = resolve }))
  view.rerender(<StrengthProgress client={{ ...client, reportPackageId: 'last' }} user={owner} onRecordAction={save} />)
  await waitFor(() => expect(progressReportPdf).toHaveBeenCalledTimes(4))
  view.unmount()
  await act(async () => last(new Blob(['late PDF'])))
