...
test('M4 owner and trainer open the current remuneration cycle and every total links to its sessions', async ({ page }) => {
  ...
  for (const id of ['u-owner', 'u-marcus']) {
    ...
    const action = cycles.locator('article').first().locator('.remuneration-cycle-actions')
    const placement = await action.evaluate(node => {
      const badge = node.querySelector('.status-badge'), button = node.querySelector('button')
      const a = badge.getBoundingClientRect(), b = button.getBoundingClientRect()
      return { adjacent: badge.nextElementSibling === button, gap: b.left - a.right, overlap: Math.min(a.bottom, b.bottom) - Math.max(a.top, b.top) }
    })
    expect(placement.adjacent).toBe(true)
    expect(placement.gap).toBeGreaterThanOrEqual(0)
    expect(placement.overlap).toBeGreaterThan(0)
...
test('M4 remuneration moves from In progress to Pending review and becomes approvable with acknowledgement alone', async ({page}) => {
  ...
  await page.evaluate(() => { location.hash = '#/remuneration/2020-09/t1' })
  await expect(page.locator('.remuneration-section-head .status-badge')).toHaveText('Pending approval')
  const pendingPill = await page.locator('.remuneration-section-head .status-badge').boundingBox()
  await page.getByRole('button', { name: 'Show remuneration amounts', exact: true }).click()
  await page.getByRole('button', { name: 'Approve Remuneration', exact: true }).click()
  const confirmation = page.getByRole('dialog', { name: 'Approve trainer remuneration?', exact: true })
  ...
  await confirmation.getByRole('button', { name: 'Approve Remuneration', exact: true }).click()
  await expect(page.locator('.remuneration-section-head .status-badge')).toHaveText('Approved')
  const approvedPill = await page.locator('.remuneration-section-head .status-badge').boundingBox()
  expect(Math.abs(approvedPill.width - pendingPill.width)).toBeLessThanOrEqual(1)
  expect(Math.abs(approvedPill.height - pendingPill.height)).toBeLessThanOrEqual(1)
  await expect(page.locator('.remuneration-section-head .status-badge')).toHaveCSS('justify-content', 'center')
