...
test('M4 owner shares the same PDF as Download and sees timestamped history after reload', async ({ page }) => {
  ...
  const share = page.getByRole('button', { name: 'Share Progress Report PDF', exact: true })
  await share.waitFor({ state: 'visible' })
  const downloadButton = page.getByRole('button', { name: 'Download Progress Report PDF', exact: true })
  ...
  await share.click()
  await expect(dialog).toHaveCount(0)
  // The visible saved audit follows asynchronous PDF inspection and sharing.
...
test('M4 trainer report activity is logged for the owner without exposing history to trainers', async ({ page }) => {
  ...
  const download = page.waitForEvent('download')
  await page.getByRole('button', { name: 'Download Progress Report PDF', exact: true }).click()
  await download
...
test('M4 unsupported file sharing offers the actual PDF for manual attachment without a popup or invented audit', async ({ page }) => {
  await fixture(page)
  await page.goto('/#/clients/c1')
  await tab(page, 'Progress')
  await page.getByLabel('Progress packages', { exact: true }).locator('article').first().getByRole('button', { name: 'View', exact: true }).click()
  await mockPdfSharing(page, { supported: false })
  const share = page.getByRole('button', { name: 'Share Progress Report PDF' })
  await share.click()
  await expect(page.getByRole('dialog')).toHaveCount(0)
  await expect(page.getByRole('alert')).toContainText('PDF sharing is unavailable in this browser')
  const downloadButton = page.getByRole('button', { name: 'Download Progress Report PDF' })
  await expect(downloadButton).toBeEnabled()
  const download = page.waitForEvent('download')
  await downloadButton.click()
  const exported = await download
  ...
  expect(events[0]).toMatchObject({ kind: 'pdf_export', clientId: 'c1', packageId: 'client-package-c1', by: { id: 'u-owner', name: 'Chau' } })
  expect(await page.evaluate(() => window.__pdfShares)).toEqual([])
})
