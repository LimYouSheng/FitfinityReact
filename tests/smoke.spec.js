...
test('client profile navigation tabs contain package session and progress data', async ({ page }) => {
  ...
  await clickProfileTab(page, 'Progress')
  await page.getByLabel('Progress packages', { exact: true }).locator('article').first().getByRole('button', { name: 'View', exact: true }).click()
  await expect(page.getByLabel('Package progress summary', { exact: true })).toHaveCount(0)
  await expect(page.locator('.strength-progress-row > span:nth-child(4)')).toHaveText(['3', '3', '3', '3', '3', '3'])
  const progressTitle = page.locator('.package-progress-title')
  await expect(progressTitle.getByRole('heading')).toHaveText('Current · 12 Sessions')
  await expect(progressTitle.getByLabel('Package progress dates')).toHaveText('24 Aug 2026 – 21 Nov 2026')
  if (page.viewportSize().width >= 700) {
    const heading = await progressTitle.getByRole('heading').boundingBox()
    const dates = await progressTitle.getByLabel('Package progress dates').boundingBox()
    expect(dates.x).toBeGreaterThanOrEqual(heading.x + heading.width)
    expect(dates.y).toBeLessThan(heading.y + heading.height)
  }
