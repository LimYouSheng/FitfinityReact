...
Updated 9 September 2026. This is the consolidated project handoff, operating guide and architecture contract. It covers the owner/trainer React portal reconstructed from the frozen v0.57 reference. Current source and later agreed requirements supersede historical prototype rules.
...
The latest verified committed remote source is `061470ceb641cbba46093deb48618352c9e1f8aa`. Its 205 file blobs match the previous renewal/service-review delivery and its GitHub Pages workflow succeeded. The current working candidate adds the refinements described below and is uncommitted. The publicly reachable deployment is a staff demo using browser-local records; real authentication and backend storage start at M5. Clients do not sign into the operational portal.
...
The required gate is **329/329 unit tests in 59/59 files; root and repository-path builds with PWA verification; 483/483 browser cases, 161 each on desktop Chromium, phone WebKit and tablet WebKit; source integrity and code health**. This candidate adds seven unit scenarios for completion presentation, summary selection, legacy completed approval, re-login and PDF sharing, plus one browser scenario across three projects for unsupported file sharing. Native-share browser tests render real PDFs and stub only the OS share boundary; they verify fresh user activation, file-only payloads and identical PDF bytes to Download. They do not certify an actual WhatsApp attachment or delivery. CI and the installer use the same exact result verifier. Inventory/discovery is not execution. Failed, skipped, flaky, retried, interrupted or incomplete tests are not green. Historical failures were repaired with regression coverage; their intermediate failure logs do not certify later source.
...
PDF file sharing depends on the browser, operating system, available share targets and secure context. Share PDF appears after generation only when the actual file passes the browser capability check. The user selects WhatsApp from the device share menu. Download PDF plus Open WhatsApp is the manual-attachment fallback; opening that chat link alone does not attach the PDF. Missing or failing native sharing cannot be resolved merely by installing the PWA. See [MDN share()](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/share).
...
- `clientService.recordProgressReportAction(clientId, {id, kind})` returns `{id, clientId, kind, at, by:{id,name}}` and a saved-action Message. New progress actions use `pdf_export` or `pdf_share_opened`; legacy `whatsapp_opened` remains readable/retryable; the UI creates a unique action ID while the adapter derives actor/time. Legacy `csv_export` entries retain their truthful label and idempotent retry, but new CSV actions are rejected. The new PDF share kind uses the same service operation and owner-only history authorization.
...
- `navigator.canShare({files:[file]})` checks actual file support. If unsupported, Download PDF saves that same prepared file and Open WhatsApp opens the client's bare chat link for manual attachment. Opening that fallback link does not automatically attach a PDF or create a PDF-share event. Render failure offers Retry PDF; closing during preparation discards the late result without sharing or logging.
